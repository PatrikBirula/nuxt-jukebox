...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Vrátíme token pro Web Playback SDK
    return {
      token: spotifyAccount.access_token
    };
  } catch (error) {
    console.error("Chyba při získávání tokenu:", error);
    throw createError({
      statusCode: 500,
      message: `Nepodařilo se získat token: ${error instanceof Error ? error.message : "Neznámá chyba"}`
    });
  }
}); 